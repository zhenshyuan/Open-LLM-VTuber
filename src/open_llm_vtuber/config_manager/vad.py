...
class SileroVADConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "orig_sr": Description(
            en="Original Audio Sample Rate", zh="原始音频采样率"
        ),
        "target_sr": Description(
            en="Target Audio Sample Rate", zh="目标音频采样率"
        ),
        ...
        "required_hits": Description(
            en="Number of consecutive hits required to consider speech", zh="连续命中次数以确认语音"
        ),
        "required_misses": Description(
            en="Number of consecutive misses required to consider silence", zh="连续未命中次数以确认静音"
        ),
...
class VADConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "silero_vad": Description(en="Configuration for Silero VAD", zh="Silero VAD 配置"),
    }
